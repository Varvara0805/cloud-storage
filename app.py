...
def get_flash_messages():
    messages_html = ''
    # Простая реализация без сложных проверок сессии
    try:
        if hasattr(app, 'flash_messages'):
            for category, message in app.flash_messages:
                if category == 'error':
...
def add_flash_message(message, category='info'):
    """Упрощенная система flash сообщений"""
        ...
        if os.path.exists(file_path):
            ...
            try:
                ...
                if current_hash != file['file_hash']:
                    add_flash_message('⚠️ File integrity check failed!', 'error')
                ...
                # Упрощенная очистка временного файла
                try:
                    import atexit
                    import threading
                    def cleanup():
